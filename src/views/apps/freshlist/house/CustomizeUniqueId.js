...
import {
  ...
  user_create_custom_id,
  View_CustomerGroup,
} from "../../../../ApiEndPoint/Api";
function CustomizeGenerateID(args) {
  const [formData, setFormData] = useState({});
  const [dynamicId, setDynamicId] = useState({});
  const [userList, setUserList] = useState([]);
  //   const [CustomerList, setCustomerList] = useState([]);
  //   const [warehouseList, setWarehouseList] = useState([]);
  //   const [transportorList, setTransportorList] = useState([]);
  const [AllListData, setAllListData] = useState([]);

  useEffect(() => {
    let userinfo = JSON.parse(localStorage.getItem("userData"));
    Apicalling(userinfo?._id, userinfo?.database);
  ...
  const Apicalling = (id, db) => {
    ...
    _Get(url, db)
      .then(res => {
        ...
        if (value?.length > 0) {
          ...
          if (formData?.selectedList == "user") {
            console.log(formData?.selectedList, "user");
            setAllListData(selected?.reverse());
          }
        }
      })
      .catch(err => {
    ...
    CreateCustomerList(id, db)
      .then(res => {
        ...
        if (value?.length) {
          ...
          if (formData?.selectedList == "customer") {
            console.log(formData?.selectedList, "customer");
            setAllListData(newarra?.reverse());
          }
        }
      })
      .catch(err => {
    ...
    _Get(Create_Warehouse_List, db)
      .then(res => {
        ...
        if (value?.length) {
          if (formData?.selectedList == "warehouse") {
            console.log(formData?.selectedList, "warehouse");
            setAllListData(value?.reverse());
            //    setWarehouseList(value.reverse());
          }
        }
      })
      .catch(err => {
    ...
    _Get(Create_Transporter_List, db)
      .then(res => {
        let value = res?.Transporter;
        if (value?.length) {
          if (formData?.selectedList == "transporter") {
            console.log(formData?.selectedList, "transporter");
            setAllListData(value?.reverse());
            // console.log("Transportor", value.reverse());
          }
        }
      })
      .catch(err => {
    ...
    Get_RoleList(id, db)
      .then(res => {
        if (formData?.selectedList == "role") {
          console.log(formData?.selectedList, "role");
          setAllListData(res?.Role?.reverse());
        }
    ...
    _Get(PurchaseProductList_Product, db)
      .then(res => {
        // Product_Title
        if (formData?.selectedList == "product") {
          console.log(formData?.selectedList, "product");
          setAllListData(res?.Product?.reverse());
        }
      })
      .catch(err => {
        console.log(err);
      });
    _Get(View_CustomerGroup, db)
      .then(res => {
        if (res?.CustomerGroup) {
          ...
          if (myActive?.length) {
            if (formData?.selectedList == "customerGroup") {
              console.log(formData?.selectedList, "customerGroup");
              setAllListData(myActive?.reverse());
            }
            console.log("CustomerGroup", myActive?.reverse());
          }
        }
      })
      .catch(err => {
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    const payload = {
      model: formData?.selectedList,
      id: formData?._id,
      userId: formData?.CustomeId,
    };

    console.log(payload);
    await _PostSave(user_create_custom_id, payload)
      .then(res => {
        console.log(res);
        if (res.status) {
  ...
  const handleSelection = async (selectedList, selectedItem, index) => {
    setFormData({ ...formData, _id: selectedItem?._id });
  };
  const onRemove1 = (selectedList, removedItem, index) => {
    console.log(selectedList);
  };
  const updatedUserList = AllListData?.map(person => {
    const {
      firstName,
      lastName,
      name,
    ...
    if (roleName) {
      fullNameArray.push(roleName);
    }
    if (Product_Title) {
      fullNameArray.push(Product_Title);
    }
    if (warehouseName) {
      fullNameArray.push(warehouseName);
    }
    if (groupName) {
      fullNameArray.push(groupName);
    }
    return {
      ...person,
      ...
      <Form onSubmit={handleSubmit}>
        <Row>
          <Col lg="3" md="3" xl="3">
            <FormGroup>
              <Label>
                Select List <span style={{ color: "red" }}>*</span>
              </Label>
              <CustomInput
                id={145}
                name="selectedList"
              ...
              >
                ...
                <option value="user">UserList</option>
                <option value="customer">CustomerList</option>
                <option value="warehouse">WarehouseList</option>
                <option value="transporter">TransportorList</option>
                <option value="customerGroup">CustomerGroupList</option>
                <option value="product">ProductList</option>
                <option value="role">RoleList</option>
              </CustomInput>
            </FormGroup>
          </Col>
          ...
          <Col lg="3" md="3" xl="3">
            <Label>
              {formData?.selectedList && formData?.selectedList
                ? ` ${formData?.selectedList}  List`
                : "List"}{" "}
            ...
            <Multiselect
              ...
              onSelect={(selectedList, selectedItem) =>
                handleSelection(selectedList, selectedItem)
              }
          ...
          <Col lg="3" md="3" xl="3">
            <FormGroup>
              <Label>
                CustomeId <span style={{ color: "red" }}>*</span>
              </Label>
              <Input
                required
                placeholder="Enter CustomeId here"
                type="text"
                name="CustomeId"
                value={formData?.CustomeId}
                onChange={handleDropdownChange}
              />
            </FormGroup>
          </Col>
...
export default CustomizeGenerateID;
...
// class CustomizeUniqueId extends React.Component {
...
//   }
// }
// export default CustomizeUniqueId;
