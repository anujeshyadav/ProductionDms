...
class WebsiteProImg extends React.Component {
  state = {
    rowData: [],
  ...
  async componentDidMount() {
    await axiosConfig.get("/getall_img").then((response) => {
  ...
  async runthisfunction(id) {
    console.log(id);
    await axiosConfig.get(`/delete_img/${id}`).then((response) => {
            ...
            <Row className="m-2">
              ...
              <Col>
                <Button
                  ...
                  onClick={() =>
                    history.push("/app/website/websiteProduct/addProImg")
                  }>
                  Add Product Image
                </Button>
              </Col>
