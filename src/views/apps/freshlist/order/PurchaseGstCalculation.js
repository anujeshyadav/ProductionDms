...
export const PurchaseGstCalculation = (Party, product, Context, Charges) => {
  ...
  product?.forEach((ele) => {
    if (typeof ele?.gstPercentage == "number") {
      ...
      if (ele?.gstPercentage?.includes("+")) {
        ele["gstPercentage"] = Number(ele?.gstPercentage?.split("+")[0] * 2);
        return Number(ele);
      } else {
        // return Number(ele);
        ele["gstPercentage"] = ele?.gstPercentage;
      }
    }
  });

  let maxGst = product.reduce(function (prev, current) {
    return prev && Number(prev?.gstPercentage) > Number(current?.gstPercentage)
      ? prev
      : current;
  });
  maxGst["gstPercentage"] = Number(maxGst?.gstPercentage);
  ...
  let Tax = {
    ...
    CgstTotal: IgstTaxType
      ...
      : Number((CgstTotal + OtherGSTCharges / 2)?.toFixed(2)),
    SgstTotal: IgstTaxType
      ? Number(SgstTotal.toFixed(2))
      : Number((SgstTotal + OtherGSTCharges / 2)?.toFixed(2)),
  };
